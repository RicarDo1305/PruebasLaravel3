            ...
            @foreach($egresados as $egresado)
             <div class="p-6 flex space-x-1 md:space-x-2">
                ...
                <x-dropdown>
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route($rutaEdit, $egresado)">
                            {{ __('Edit') }}
                        </x-dropdown-link>
                        <form method="POST" action="{{ route($rutaDestroy, $egresado) }}">
                            @csrf @method('DELETE')
                            <x-dropdown-link :href="route($rutaDestroy, $egresado)" onclick="event.preventDefault();
