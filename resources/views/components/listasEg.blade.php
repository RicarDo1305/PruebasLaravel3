<div class="mt-6 bg-slate-800 shadow-sm rounded-lg divide-y divide-gray-900 m-2">
    ...
    <div class="p-6 flex space-x-1 md:space-x-2">
        <svg class="h-4 w-4 md:h-6 md:w-6 text-gray-400 -scale-x-100" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
        </svg>
        <div class="flex-1">
            <div class="flex justify-between items-center">
                <div>
                    <span class="text-gray-200 text-sm md:text-lg">{{ $egresado->name }}</span>
                    <small class="ml-2 text-xs md:text-sm text-gray-400">{{ $egresado->created_at->format('j M Y, g:i a') }}</small>
                ...
                <x-dropdown>
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route($rutaEdit, $egresado)" onclick="stayOpen(event)">
                            {{ __('Edit') }}
                        </x-dropdown-link>
    ...
    <x-comprobar-eliminacion :rutaEliminar=$rutaDestroy :tipo=$egresado />
    @endforeach
</div>
