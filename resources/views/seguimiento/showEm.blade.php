            ...
            @foreach($empleadores as $empleador)
             <div class="p-6 flex space-x-2">
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <small class="ml-2 text-sm text-gray-400">{{ $empleador->created_at->format('j M Y, g:i a') }}</small>
                            @if($empleador->created_at != $empleador->updated_at)
                            <small class="text-sm text-gray-400"> &middot; {{ __('Edited') }}</small>
                            @endif
                        </div>
                    </div>
                    <p class="mt-4 text-lg text-gray-100">{{ $empleador->ubicacion }}</p>
                </div>
                ...
                <x-dropdown>
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('seguimiento.listaEm.edit', $empleador)">
                            {{ __('Editar empleador') }}
                        </x-dropdown-link>
                        <form method="POST" action="{{ route('seguimiento.listaEm.destroy', $empleador) }}">
                            @csrf @method('DELETE')
                            <x-dropdown-link :href="route('seguimiento.listaEm.destroy', $empleador)" onclick="event.preventDefault();
